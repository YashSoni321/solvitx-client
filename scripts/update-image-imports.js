...
const updateFileImports = (filePath) => {
  try {
    ...
    Object.entries(imageMapping).forEach(([original, optimized]) => {
      const importRegex = new RegExp(`from\\s+["']([^"']*${original.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})["']`, 'g');
      const srcRegex = new RegExp(`src=["']([^"']*${original.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})["']`, 'g');
      const backgroundImageRegex = new RegExp(`backgroundImage=\\{[^}]*${original.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[^}]*\\}`, 'g');
...
const main = () => {
  ...
  console.log('   2. Check browser DevTools to verify WebP images are being used');
